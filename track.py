...
for i in range(199):
    tp.annotate(f.loc[f["frame"] == i], frames[i]
                ).figure.savefig("tempframes/f" + str(i))
...
out = cv2.VideoWriter(
    new_video_name, cv2.VideoWriter_fourcc(*'DIVX'), 15, size)
...
def condition(x): return ((x['mass'].mean() > 825) & (x['size'].mean() < 8) &
                          (x['ecc'].mean() > 0.1))
